...
router.post("/auth/signup", async (req, res) => {
  if (!req.body.namelogin || !req.body.password) {
    ...
    try {
      ...
      await newUser.save();
      let token = jwt.sign(newUser.toJSON(), process.env.SECRETORKEY, {
...
router.post("/auth/login", async (req, res) => {
  try {
    let foundUser = {};
    if (req.body.email) {
      foundUser = await User.findOne({ email: req.body.email }).populate(
        "address"
      );
    } else {
    ...
    if (!foundUser) {
      ...
      if (foundUser.comparePassword(req.body.password)) {
        let token = jwt.sign(foundUser.toJSON(), process.env.SECRETORKEY, {
